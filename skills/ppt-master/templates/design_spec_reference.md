...
| **Reading Mode** | [`text` read-close / `balanced` business / `presentation`; Confirm UI compatibility key: `delivery_purpose`, execution-lock key: `consumption_mode`. Defines how meaning is divided among the page, visuals, presenter, and notes; therefore drives page grammar, granularity, density / rhythm, page-count recommendation, and note burden. The body baseline is a downstream typography default, not the definition shown to the user. See strategist.md §6.1.] |
...
> Step 4 Confirm UI: present **≥3** typography candidates (creative recommendations always offer real choice — same rule as h.5; fewer only on the honest-shortfall exception, with a stated reason), each splitting CJK + Latin for `heading` and `body` (with `css` preview stacks), in `confirm_ui/recommendations.json`. The Stage-2 reading mode supplies one shared initial `body_size` baseline in **px** (the system's only unit, every canvas); switching font / direction cards preserves the current sizing state. The confirmed `result.json` carries px directly — no conversion. Schema: [`scripts/docs/confirm_ui.md`](../scripts/docs/confirm_ui.md).
...
> **Baseline selection**: after reading mode has made the information-carriage decision, the Confirm UI uses it to seed one fixed body baseline and derives unpinned role sizes locally. This is a typography default, not part of the reading-mode label or card description. Communication intent, content density, and visual style drive page treatment / rhythm / the *other* roles, **not** the body size.

**Baseline (unitless px)**: Body font size = [fill in]. For PPT 16:9, the initial typography default is **`text`** `20`, **`balanced`** `24` (default), **`presentation`** `32`. The browser applies `reading mode → body baseline → unpinned role sizes` locally without regenerating Stage 2. A user-edited body or role value is pinned; every confirmed `result.json typography.body_size` / `sizes` value is already px and becomes the locked slot verbatim. For non-PPT canvases, author px from the confirmed canvas scale (see [strategist.md §g](../references/strategist.md) per-canvas table).
